...
internal sealed class UsersEndpoint(HttpClient http, Subject<HelixError> errors)
    : HelixEndpoint(http, errors), IUsersEndpoint
{
    ...
    public async Task BlockAsync(string targetUserId, CancellationToken ct = default)
        => await PutAsync($"/helix/users/blocks?target_user_id={Uri.EscapeDataString(targetUserId)}", ct);
